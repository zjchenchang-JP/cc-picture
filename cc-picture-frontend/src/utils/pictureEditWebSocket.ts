// 图片编辑 WebSocket 连接的工具类
// 自定义事件监听机制，使用工具类的组件可以通过 on 方法注册事件处理函数，然后通过 triggerEvent 函数触发事件处理函数
export default class PictureEditWebSocket {
   ...
   */
  connect() {
    // 本地地址（直连后端 8123：与登录请求同域，浏览器才会自动携带 JSESSIONID，握手才能通过鉴权）
    const DEV_BASE_URL = 'ws://localhost:8123'
    // 线上地址
    // const PROD_BASE_URL = "ws://81.69.229.63";
    // 注意：后端 application.yml 配了 context-path: /api，WebSocket 的真实路径也要带 /api 前缀
